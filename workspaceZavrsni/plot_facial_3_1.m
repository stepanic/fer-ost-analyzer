function plot_facial_3_1(normal_array, tl, time, tl_end)

% plot_facial_3_1(eye, time_3, fac_time)
...
ymin = min(normal_array) * 0.99;
ymax = max(normal_array)* 1.01;

figure();
hold on
plot(time, normal_array)

% plot((1:len1),b1)
% hold on

for x=tl
    line('XData', [x x], 'YData', [ymin ymax], 'LineStyle', '-', 'LineWidth', 2, 'Color','r')
end
% plot(((len1+1):(len1+len2)),n1)
% line('XData', [tl(2) tl(2)], 'YData', [ymin ymax], 'LineStyle', '-', 'LineWidth', 2, 'Color','r')
% plot(((len1 + len2 +1):(len1+len2+ len3)),s)
% line('XData', [tl(3) tl(3)], 'YData', [ymin ymax], 'LineStyle', '-', 'LineWidth', 2, 'Color','r')
% plot(((len1 + len2 + len3 +1):(len1+len2+ len3 + len4)),n2)
% line('XData', [tl(4) tl(4)], 'YData', [ymin ymax], 'LineStyle', '-', 'LineWidth', 2, 'Color','r')
% plot(((len1 + len2 + len3 + len4 +1):(len1+len2+ len3 + len4 + len5)),b2)
% line('XData', [tl(5) tl(5)], 'YData', [ymin ymax], 'LineStyle', '-', 'LineWidth', 2, 'Color','r')
